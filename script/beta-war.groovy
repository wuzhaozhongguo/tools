            ...
            node("${__node}") {
                ...
                if (!__targets){//没有目标机器，发布在facade
                    ...
                    for (def __targetNode in __targets){
                        ...
                        sh "ssh jhd@${__targetNode} 'mkdir -p ${_config.jenkins.remote_work_path}'"//创建工作目录
                        sh "ssh jhd@${__targetNode} 'mkdir -p ${_config.project.path}'"//创建项目目录
                        sh "ssh jhd@${__targetNode} 'mkdir -p ${_config.jenkins.remote_work_path}/${_config.package.folder_name}/target/'"//创建包文件所在目录
                    ...
                    '''+
                        ...
                        sh "scp ${_config.package.folder_name}/target/${_config.package.name} ${_config.jenkins.user}@${__targetNode}:${_package_path}${_config.package.folder_name}/target/"//传包到目标服务器
